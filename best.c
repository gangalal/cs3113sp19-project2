...
struct _process{ //to hold the process 
	char processName[20];
	int process;
};
...
int findKey(int Array1[], int key, int max);//finds the closest big enough block
...
int main (int argc, char **argv)
{
	...
	process* library = (process*) malloc(totalAmount * sizeof(process));//allocates memory dynamically
		...
		if(strstr(line, "#") == NULL)
		{
			if(strstr(line, "REQUEST") != NULL)
			{
				...
				else
				{
					...
					int key = findKey(freeBlocks, atoi(temp[2]), blockSize);//finds the best key


					for(int i = 0; i<blockSize; i++)
					{

						if (key ==library[i].process)
						{
							printf("ALLOCATED %s %d\n", temp[1], i);
